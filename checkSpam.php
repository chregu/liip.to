...
//check( "(lastCheck = '0000-00-00 00:00:00' OR DATE_ADD(lastCheck, INTERVAL 12 HOUR) < now())");
...
// neuer als 21 Tage => alle 8 Stunden
check( " DATE_ADD(added, INTERVAL 21 DAY) > now() AND DATE_ADD(lastCheck, INTERVAL 8 HOUR) < now() ");
...
check( " DATE_ADD(added, INTERVAL 365 DAY) > now() AND DATE_ADD(lastCheck, INTERVAL 28 DAY) < now() ");

function check($where) {
   	$c = new UrlCheck();
$dbh = new PDO('mysql:host=localhost;dbname=liip_to', "liip_to", "oP3phuki");
foreach ($dbh->query("SELECT * from urls WHERE lastCheck = '0000-00-00 00:00:00' OR (  $where ) order by RAND() LIMIT 100; ") as $row) {
$reason = array();
print $row['url'] ."\n";
